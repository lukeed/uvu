...
not.rejects = async function (blk, exp, msg) {
	...
	try {
		...
		if (typeof exp === 'function') {
			assert(!exp(err), true, false, 'not.rejects', false, 'Expected promise not to reject matching exception', msg);
		} else if (exp instanceof RegExp) {
			...
			assert(false, true, false, 'not.rejects', false, 'Expected promise not to reject', msg);
		}
	}
}
